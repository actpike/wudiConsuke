...
### v1.0.3 (2025/01/24)
- CSV対応データエクスポート・インポート機能追加
- 作品評価データをCSV形式で出力（Excel対応、年度情報明記）
- JSONとCSVの形式選択プルダウン追加
- CSV形式での作品データ取り込み機能

### v1.0.2 (2025/01/24)
- 複数年度対応システム実装（年度別データ管理、年度選択UI）
- YearManagerクラス追加（年度別データ操作、テスト年度サポート）
- 年度データ削除機能追加（安全な削除処理、確認ダイアログ、復活防止機能）
- Web監視機能の年度別URL対応
- 既存データの安全な新構造移行機能
- 年度削除後の復活バグ修正（削除済み年度履歴管理）
